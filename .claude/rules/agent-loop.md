...
장시간 무인 실행을 위해 `loop-orchestrator` 가 한 tick 의 모든 결정을 담당하고, `loop-monitor` 가 매 tick 끝에 건강 체크를 수행. `/journey-loop` 호출 시 메인 세션은 stop-check + orchestrator spawn + monitor spawn + schedule 만 수행. orchestrator 가 Phase A (tester→gap-planner→plan-implementer fall-through) + Phase B (PM drainage) 를 실행하고 요약을 monitor 에 전달, monitor 는 anomaly 감지 후 auto-fix 또는 user 에게 escalate.
...
| [`loop-orchestrator`](../agents/loop-orchestrator.md) | `/journey-loop` 이 empty prompt 로 spawn | 한 tick 의 Phase A decision tree + Phase B PM drainage 실행. state 재도출 후 최대 2개 sub-agent spawn. stateless across ticks | 1 개 단일 tick summary block | 파일 수정 없음 (spawn 한 sub-agent 가 각자 소유 범위 수정) |
...
열한 agent 는 서로 직접 통신하지 않습니다 (예외 두 가지: `loop-orchestrator` 가 Phase A/B sub-agent 를 spawn, `loop-monitor` 가 안전 auto-fix 에서 `project-manager` 를 재invoke — 그 외는 모두 문서가 공용 큐):
