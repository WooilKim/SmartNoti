# Six-agent Improvement Loop

SmartNoti 는 여섯 개의 사용자 정의 subagent 로 계속 개선되는 구조입니다. 이 문서는 여섯 agent 가 어떻게 협조하는지 한 페이지로 정리합니다 — 새 세션/새 에이전트가 들어와도 이 그림만 읽으면 루프를 이어갈 수 있게.
...
여섯 agent 는 서로 직접 통신하지 않습니다. 문서가 공용 큐 역할을 합니다:
                        ...
                        └────────┬────────┘
                         ...
                         approve / request-changes
                                 ...
                                 └─▶ 사람 또는 agent 자신이 merge
                                          ...
                                          ▼
                    ┌───────────────────────────────┐
                    │        loop-manager           │  ◀── 사용자 (/loop-status)
                    │  status · stop · resume · tune│     감사 로그만 읽어 dashboard
                    └───────────────────────────────┘
```
...
- **수동 (v0)**: 사용자가 `/journey-test`, `/ui-inspect`, `/gap-plan`, `/plan-implement`, `/pr-review`, `/loop-status` 로 직접 호출.
