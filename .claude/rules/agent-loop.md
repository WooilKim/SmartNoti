# Five-agent Improvement Loop

SmartNoti 는 다섯 개의 사용자 정의 subagent 로 계속 개선되는 구조입니다. 이 문서는 다섯 agent 가 어떻게 협조하는지 한 페이지로 정리합니다 — 새 세션/새 에이전트가 들어와도 이 그림만 읽으면 루프를 이어갈 수 있게.
...
다섯 agent 는 서로 직접 통신하지 않습니다. 문서가 공용 큐 역할을 합니다:
...
gap-planner       plan-implementer      journey-tester   ui-ux-inspector
 ...
 plans/*.md ──▶ 코드 + journeys/*.md ──▶ journeys/*.md + Known gaps ◀──┘
                                ...
                                ▼
                        ┌─────────────────┐
                        │ project-manager │  ◀── 모든 agent-origin PR
                        │ (review gate)   │        in review queue
                        └────────┬────────┘
                                 ▼
                         approve / request-changes
                                 │
                                 └─▶ 사람 또는 agent 자신이 merge
```

- **행동 드리프트** (recipe ≠ 실제) 는 `journey-tester`, **시각 드리프트** (UI ≠ `ui-improvement.md`) 는 `ui-ux-inspector` 가 감지. 둘 다 Known gaps 큐에 쌓아 `gap-planner` 가 소화.
- **PR 심사**는 `project-manager` 가 담당 — 다른 agent 가 연 PR 의 scope / traceability / CI / carve-out 을 확인하고 `--approve` 또는 `--request-changes` 를 공식 vote 로 남김.
- `docs-sync.md` 가 다섯 agent 의 공통 규약. 모두 system prompt 에서 이 규칙을 읽고 따릅니다.
- 각 agent 는 **PR 을 여는 것이 최대** (`project-manager` 는 리뷰까지). `main` 에 직접 push 금지, merge 는 자기 scope 안에서만 허용 (`journey-tester` / `ui-ux-inspector` 는 docs-only self-merge, 나머지는 사람 merge).

## 오케스트레이션 옵션

- **수동 (v0)**: 사용자가 `/journey-test`, `/ui-inspect`, `/gap-plan`, `/plan-implement`, `/pr-review` 로 직접 호출.
