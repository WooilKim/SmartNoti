# Twelve-agent Improvement Loop

SmartNoti 는 열두 개의 사용자 정의 subagent 로 계속 개선되는 구조입니다. 이 문서는 열두 agent 가 어떻게 협조하는지 한 페이지로 정리합니다 — 새 세션/새 에이전트가 들어와도 이 그림만 읽으면 루프를 이어갈 수 있게. 시스템 자체도 `loop-retrospective` 가 주기적으로 돌아보며 meta-plan 으로 개선 제안을 올리기 때문에, **구조는 고정이 아니라 자기 진화함**.
...
열두 agent 는 서로 직접 통신하지 않습니다 (예외 세 가지: `/journey-loop` wrapper 가 Phase A/B sub-agent + monitor 를 직접 spawn (이전 `loop-orchestrator` 자리), `loop-monitor` 가 안전 auto-fix 에서 `project-manager` 를 재invoke, `/journey-loop` wrapper 가 plan 이 status:shipped 로 flip 된 tick 에서 `feature-reporter` 를 spawn — 그 외는 모두 문서가 공용 큐):
