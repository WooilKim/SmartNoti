...
열한 agent 는 서로 직접 통신하지 않습니다 (예외 두 가지: `/journey-loop` wrapper 가 Phase A/B sub-agent + monitor 를 직접 spawn (이전 `loop-orchestrator` 자리), `loop-monitor` 가 안전 auto-fix 에서 `project-manager` 를 재invoke — 그 외는 모두 문서가 공용 큐):
