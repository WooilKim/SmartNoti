...
| [`loop-monitor`](../agents/loop-monitor.md) | orchestrator 요약 텍스트 또는 empty | 매 tick 끝에 anomaly rubric (PM_SKIPPED / AUDIT_DRIFT / STUCK_PR / SUB_AGENT_ERROR / REPEAT_NOOP_STORM / SELF_MOD_PENDING / CI_RED_WITHOUT_VERDICT) 적용. enumerated safe case 는 auto-fix (PM 재invoke / audit backfill PR), 나머지는 user 로 escalate | ≤ 150-word health report + `docs/loop-monitor-log.md` 행. 선택적 audit-backfill PR | `docs/loop-monitor-log.md` + 선택적 ops PR (never self-merges, never touches `.claude/`) |
