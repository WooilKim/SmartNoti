...
> **SUPERSEDED 2026-04-27** — PR #486 에서 사용자의 real-device (Galaxy S24 `R3CY2058DLJ`) ADB dump + SQLite inspection 으로 가설 (extended content fields 미추출) 이 invalidated 됨. body 에 `(광고)` 가 정상적으로 들어와 있고 KEYWORD 매치도 됐으나, ① IMPORTANT 키워드 (배송/결제/대출/출발) 가 본문에 동시 등장해 `CategoryConflictResolver` 의 sortOrder 가 IMPORTANT 를 winner 로 선택, ② PROMO Category 의 default action=SILENT 라서 `SuppressedSourceAppsAutoExpansionPolicy` 가 fire 안 함. 후속 plan: [`docs/plans/2026-04-27-fix-issue-478-promo-prefix-precedence-and-bundle-by-default.md`](2026-04-27-fix-issue-478-promo-prefix-precedence-and-bundle-by-default.md).
