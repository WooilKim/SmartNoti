...
- (partially resolved 2026-04-27, plan `2026-04-27-refactor-settings-screen-split`) `app/src/main/java/com/smartnoti/app/ui/screens/settings/SettingsScreen.kt` was 2244 lines; refactor extracted four sub-files (`SettingsQuietHoursSection.kt` / `SettingsDeliveryProfileSection.kt` / `SettingsDuplicateSection.kt` / `SettingsOperationalSection.kt`) bringing root file to 1118 lines. Residual 1118 lines is the SettingsScreen root composable + the suppression-management / app-selection / notification-access cluster (~750 lines) which a follow-up plan should address. Behavior unchanged — characterization test stayed GREEN through every extraction. → follow-up plan: `docs/plans/2026-04-27-refactor-settings-suppression-cluster-split.md`
