...
- 2026-04-27: **#511 fix shipped — Quiet Hours 발화 SILENT 의 트레이 중복 해소** (PR #513 `5db1208` + closure PR #516). Quiet Hours 가 쇼핑 알림을 SILENT 로 떨어뜨리는 분기 (Observable step 3 후속) 에서, 이전 `SourceNotificationRoutingPolicy` 의 `SilentMode == ARCHIVED → cancel = false` 분기가 source 알림을 트레이에 잔존시켜 `smartnoti_silent_group` child 와 함께 4–6 entry 의 Railway-style pile 을 만들던 증상이 사라졌다. Option C invariant 적용 후 모든 SILENT decision 이 `cancelSourceNotification = true AND notifyReplacementNotification = true` 로 라우팅되어, Quiet Hours 가 발화한 쇼핑 알림 1건당 트레이에는 SmartNoti `smartnoti_silent_group_*` 또는 `smartnoti_silent_summary` replacement 만 남는다 (silent-auto-hide journey 의 Observable step 1-2 와 동일 invariant). Verification recipe (`am broadcast --es package_name com.coupang.mobile`) 은 변경 없음 — debug-inject receiver 는 트레이 게시 경로를 거치지 않으므로 quiet-hours 분기 발화 + DB row 저장 만 검증한다 (Verification recipe section 4 의 SQL 결과는 동일). 트레이 중복 검증은 silent-auto-hide journey 의 R3CY2058DLJ 라이브 캡처가 cross-cuts (같은 라우팅 코드 패스). Code pointer 변경 없음 (분기는 `SourceNotificationRoutingPolicy` 안에 머물고 quiet-hours pipeline 은 그대로). `last-verified` 2026-04-27 → 2026-04-27 unchanged (same day; 본 변경은 다른 분기의 결과물이며 quiet-hours recipe 자체 재실행 없음).
