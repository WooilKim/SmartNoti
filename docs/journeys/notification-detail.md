...
4. 존재하면 섹션 순서대로 렌더링:
   ...
   - (조건부) **발신자 분류 제안 카드** — `SenderRuleSuggestionCard`. `Settings.senderSuggestionEnabled` (default ON) + 매칭 SENDER 룰 부재 + title 비어있지 않음 + length ≤ 60 + system-deny-list 미매치 5 가드를 통과할 때만 노출. body=`이 발신자를 항상 [중요]로 분류할까요?\n"<title>"`, 두 버튼 (`예, 중요로` / `무시`). `[예]` 탭 시 `AcceptSenderSuggestionUseCase` 가 SENDER 룰 (matchValue=title) 을 upsert + PRIORITY Category (`cat-onboarding-important_priority` 우선, 없으면 가장 낮은 `order` 의 PRIORITY) 의 `ruleIds` 에 dedup append + snackbar `"같은 발신자의 다음 알림부터 중요로 분류돼요"`. PRIORITY Category 자체가 없으면 `"중요 분류가 없어 발신자 규칙을 만들 수 없어요. 분류 변경에서 새 분류를 만들어 주세요."` snackbar 후 룰 미저장. `[무시]` 는 in-memory dismiss (다음 진입에서 다시 노출 가능).
...
6. 재분류 효과는 **즉시** 관측됨 — Path A / Path B 모두 시트/에디터 종료 직후 `ApplyCategoryActionToNotificationUseCase` 가 대상 알림 row 자체의 `status` 를 매핑된 Category action 으로 (PRIORITY/DIGEST/SILENT/IGNORE), `reasonTags` 에 `사용자 분류` 를 dedup append 로 갱신한다. Home / 정리함 / Inbox 의 row 는 즉시 새 status 로 이동/재분류된다. 추가로 동일 sender/앱으로 들어오는 후속 알림도 새로 묶인 Category 의 action 으로 자동 분류된다 (→ [notification-capture-classify](notification-capture-classify.md)) — 이건 별개 효과.
...
- NotificationRepository: 대상 알림 row 의 `status` 가 매핑된 Category action 으로 즉시 갱신되고 (`PRIORITY → PRIORITY` / `DIGEST → DIGEST` / `SILENT → SILENT` / `IGNORE → IGNORE`, mapper: `CategoryActionToNotificationStatusMapper`), `reasonTags` 에 `사용자 분류` 가 dedup append 된다. 후속 동일 sender/앱 알림 자동 재분류는 그대로 별개 효과로 남는다.
...
- `ui/screens/detail/NotificationDetailScreen` — 단일 "분류 변경" Button + `CategoryAssignBottomSheet` 호스팅 + Path A/B 후 confirmation `SnackbarHost` (BottomNav 인셋 위에 띄움) + `SenderRuleSuggestionCard` 조건부 호스팅 (`SenderRuleSuggestionCardSpec.shouldShow` 5 가드 통과 + per-entry dismiss flag false 시)
- `ui/screens/detail/SenderRuleSuggestionCard` + `SenderRuleSuggestionCardSpec` — Detail 의 발신자 분류 제안 카드 + 노출 조건 / 카피 / PII heuristic SSOT
- `domain/usecase/AcceptSenderSuggestionUseCase` — `[예, 중요로]` 탭 시 SENDER 룰 upsert + PRIORITY Category 의 `ruleIds` dedup append. Destination 우선순위: `cat-onboarding-important_priority` → 가장 낮은 `order` PRIORITY Category → `Outcome.NoPriorityCategory` (룰 미저장)
...
- `domain/usecase/ApplyCategoryActionToNotificationUseCase` — Path A/B 호출 후 대상 알림 row 의 `status` + `reasonTags` 즉시 갱신 (race-safe: destination action null 시 row write skip)
- `domain/usecase/CategoryActionToNotificationStatusMapper` — `CategoryAction → NotificationStatusUi` 4-case `when`
...
- `CategoryActionToNotificationStatusMapperTest` — 4-case `when` 핀 (PRIORITY / DIGEST / SILENT / IGNORE)
- `ApplyCategoryActionToNotificationUseCaseTest` — status 매핑 + `사용자 분류` dedup append + 단일 write count + 다른 필드 보존
- `SenderRuleSuggestionCardSpecTest` — 5 가드 + body / 라벨 카피 핀
- `AcceptSenderSuggestionUseCaseTest` — destination ladder (`cat-onboarding-important_priority` 우선 / fallback / `NoPriorityCategory`) + blank-title 거부 + title trim + 결정적 id idempotency
...
# 4. 경로 A — 기존 Category 하나 탭 → 시트 닫힘 + snackbar
#    → 뒤로가기로 list 복귀 시, 같은 row 의 StatusBadge 가 새 Category 의 action 으로 즉시 변경됨
#      (예: PRIORITY Category 면 `즉시 전달` / Home Priority 섹션 등장)
#    → 후속 동일 sender 알림도 해당 Category 의 action 으로 자동 분류되는지 관측
...
#    → 저장 시 Rule+Category 동시 persist + 같은 row 의 StatusBadge 가 새 Category 의
#      action 으로 즉시 변경됨

# 5a. 경로 B cancel — editor 에서 "닫기" → snackbar 미등장 + row 상태 변경 없음

# 6. (선택) IGNORE Category Path A — IGNORE action Category 생성 후 같은 row 할당
#    → 기본 뷰에서 즉시 사라짐. Settings "무시된 알림 아카이브 표시" 토글 ON 시
#      [ignored-archive] 화면에 등장. Detail 자체는 살아 있어 back 시 list 로 복귀.

# 검증 안내 — 위 4 / 5 단계의 row 즉시 갱신 (post-#377/#378/#379) 은
# 별도 ADB 자동화 없이 시각 확인. 다음 journey-tester sweep 에서 재검증.
```
...
- 2026-04-26: **Detail "이 알림도 지금 재분류" inline CTA** — Path A (기존 Category 탭) / Path B (`CategoryEditorScreen.onSaved` 신규 Category) 두 경로 모두 시트/에디터 dismiss 직후 신규 `ApplyCategoryActionToNotificationUseCase` 가 대상 알림 row 의 `status` 를 매핑된 Category action 으로, `reasonTags` 에 `사용자 분류` 라벨을 dedup append 한다 (mapper: 신규 `CategoryActionToNotificationStatusMapper` — `PRIORITY/DIGEST/SILENT/IGNORE` 4-case `when`). Path A 는 categories snapshot 을 tap 시점에 capture 해 race-safe lookup (Category race deletion 시 row write skip + snackbar 만). Path B 는 `onSaved(Category)` 의 just-persisted action 을 권위로 사용. legacy 4-버튼 layout 의 즉시 상태 갱신 시맨틱이 redesign 이후 누락됐던 UX 후퇴를 보완 — Home/정리함 list 가 사용자 의도를 즉시 반영. snackbar 카피는 그대로. Plan: `docs/plans/2026-04-26-detail-reclassify-this-row-now.md` Tasks 1 (#377), 2-3 (#378), 4-5 (#379), 6-8 (this PR). `last-verified` 는 미bump — 다음 journey-tester sweep 에서 row badge 즉시 변경을 재검증.
- 2026-04-27: journey-tester end-to-end re-verify (emulator-5554, APK `lastUpdateTime=2026-04-27 11:56:47`, post-#377/#378/#379) — `cmd notification post -S bigtext -t 'DetailTest_0427' FbTest1 '분류 변경 테스트'` baseline → 정리함 → Shell digest 묶음 → preview row 탭 → Detail 마운트. Observable steps 1–6 PASS: `DetailTopBar` "알림 상세" + 뒤로가기, 알림 요약 (Shell / Coupang / 오늘의 / `StatusBadge=Digest`), "왜 이렇게 처리됐나요?" + "적용된 규칙" 서브섹션 (`반복 알림` clickable rule chip), "어떻게 전달되나요?" 5 라벨 (Digest 묶음 전달 / 조용히 표시 / 진동 없음 / Heads-up 꺼짐 / 잠금화면 내용 일부), "원본 알림 처리 상태" + 2 라벨, "이 알림 분류하기" + 단일 `Button("분류 변경")`. 버튼 탭 → `CategoryAssignBottomSheet` (드래그 핸들 + 타이틀 "분류 변경" + 부제 + "기존 분류에 포함" 헤더 + 4 Category row [중요 알림 / 프로모션 알림 / 반복 알림 / BulkNewCat] + terminal `+ 새 분류 만들기`). 경로 A "중요 알림" 탭 → 시트 dismiss + snackbar `"중요 알림 분류로 옮겼어요"` 렌더 (bounds [74,1927][1006,2053]) + 같은 row 의 reasonTags 에 `사용자 분류` 라벨 즉시 append + "어떻게 전달되나요?" 카피가 `Digest 묶음 전달` → `즉시 전달` 로 즉시 전환 (Observable step 6 — `ApplyCategoryActionToNotificationUseCase` 의 row write 실행 증명). DRIFT 없음. `last-verified` 2026-04-26 → 2026-04-27.
...
- 2026-04-28: **발신자 분류 제안 카드 (SenderRuleSuggestionCard) 추가** — issue #526 P1 fix Tasks 7-9. `NotificationDetailScreen` 의 LazyColumn 에 `NotificationDetailReclassifyCta` 위 위치로 신규 카드를 호스팅. 노출 5 가드 (`Settings.senderSuggestionEnabled` ON / 매칭 SENDER 룰 부재 / title 비어있지 않음 / length ≤ 60 / system-deny-list 미매치) 를 `SenderRuleSuggestionCardSpec.shouldShow` 가 응축. `[예, 중요로]` 탭 시 `AcceptSenderSuggestionUseCase` 가 SENDER 룰 (matchValue=title.trim()) 을 upsert + 시드된 `cat-onboarding-important_priority` (없으면 가장 낮은 `order` PRIORITY Category) 의 `ruleIds` 에 dedup append + snackbar `"같은 발신자의 다음 알림부터 중요로 분류돼요"`. PRIORITY Category 부재 시 `"중요 분류가 없어 발신자 규칙을 만들 수 없어요. 분류 변경에서 새 분류를 만들어 주세요."` snackbar 후 룰 미저장. `[무시]` 는 in-memory dismiss (per-Detail-entry; 다음 진입에서 다시 노출 가능). 메신저 1:1 DM (Teams / Slack / KakaoTalk) 에서 sender 메타데이터가 비어있어 PERSON 룰로 잡히지 않는 케이스를 1 탭으로 PRIORITY 로 올리는 단축 경로 — Settings → 룰 관리 의 5+ 탭 경로 우회. ADB e2e on emulator-5554: `cmd notification post -t 김동대SpecialRecon TeamsDM526 ...` → 검토 대기 row 탭 → Detail 진입 → 카드 렌더 (body=`이 발신자를 항상 [중요]로 분류할까요?\n"김동대SpecialRecon"`) → `[예, 중요로]` 탭 → snackbar + DataStore `smartnoti_rules` 에 `sender:김동대SpecialRecon|...|SENDER|true|김동대SpecialRecon` row + `cat-onboarding-important_priority` 의 ruleIds 에 새 id append → 후속 동일 title 알림 (`TeamsDM527`) 자동 PRIORITY 분류 PASS. Plan: `docs/plans/2026-04-28-fix-issue-526-sender-aware-classification-rules.md` Tasks 7-9 (this PR), Tasks 1-2 (#536), Tasks 3-4 (#540), Tasks 5-6 (#541). `last-verified` 변경 없음 — 다음 journey-tester sweep 에서 full recipe 재검증.
- 2026-04-28: journey-tester end-to-end re-verify (emulator-5554, APK `lastUpdateTime=2026-04-28 13:01:40`, post-#543) — `cmd notification post -S bigtext -t 'DetailTest_0428' FbTest1_0428 '분류 변경 테스트'` → 검토 대기 → row 탭 → Detail 마운트. Observable steps 1–6 모두 PASS. **신규 SenderRuleSuggestionCard 검증**: card 가 reclassify CTA 위에 호스팅 (bounds [42,1620][1038,1989] vs CTA [42,2031][1038,2127]), body=`이 발신자를 항상 [중요]로 분류할까요?\n"DetailTest_0428"` (line break + quoted title — doc 카피와 정확히 일치), 두 버튼 `[예, 중요로]` ([95,1811][529,1937]) + `[무시]` ([550,1811][985,1937]) 렌더. `[예, 중요로]` 탭 → snackbar `같은 발신자의 다음 알림부터 중요로 분류돼요` (Detail 화면 하단) + DataStore 검증: `smartnoti_rules` 에 `sender:DetailTest_0428|DetailTest_0428 발신자||SENDER|true|DetailTest_0428|...` upsert 확인 + `smartnoti_categories` 의 `cat-onboarding-important_priority` ruleIds 끝에 `sender:DetailTest_0428` append 확인 (`AcceptSenderSuggestionUseCase` destination ladder Outcome.Saved 경로 증명). 후속 동일 title 알림 (`FbTest2_0428` "후속 알림 테스트") 자동 PRIORITY 분류 + `사용자 규칙` reasonTag PASS. DRIFT 없음. `last-verified` 2026-04-27 → 2026-04-28.
- 2026-04-27: **NotificationDetailScreen 파일 분해 (refactor)** — 687 lines 의 root 파일을 같은 패키지의 sub-file 3개 (`NotificationDetailReasonSection.kt` / `NotificationDetailDeliverySection.kt` / `NotificationDetailReclassifyActions.kt`) 로 분해. Root file 271 lines (state hoisting + LazyColumn 골격 + sub-section 호출만 남음). 동작 변경 없음 — `NotificationDetailScreenCharacterizationTest` (32 케이스) GREEN, full unit test suite GREEN. Code pointers 의 `ui/screens/detail/NotificationDetailScreen` 경로는 그대로 유효 (외부 import 영향 없음). Plan: `docs/plans/2026-04-27-refactor-notification-detail-screen-split.md` Tasks 2–4 (this PR), Task 1 (#465). `last-verified` 변경 없음 — 순수 리팩터.
