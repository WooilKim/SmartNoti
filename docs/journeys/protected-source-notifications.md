...
- 2026-04-28: **Issue #524 cross-cut — 트레이 정리 경로도 PERSISTENT_PROTECTED 계약을 따른다**. 신규 `TrayOrphanCleanupRunner` (Settings → "트레이 정리" 카드) 가 active StatusBarNotification snapshot 을 단일 pass 로 순회하면서 `Notification.flags & (FLAG_FOREGROUND_SERVICE | FLAG_NO_CLEAR | FLAG_ONGOING_EVENT) != 0` 인 entry 는 cleanup 후보에서 제외하고 `result.skippedProtectedCount` 로 보고 + logcat warn 한 줄 (`TrayOrphanCleanupRunner` tag) 을 남긴다. 음악 (`FLAG_NO_CLEAR | FLAG_ONGOING_EVENT`) / 통화 / 내비 / 다운로드 (`FLAG_FOREGROUND_SERVICE`) 의 source 알림이 사용자 trigger cleanup 으로도 끊기지 않는 것을 보장 — 본 journey 의 PERSISTENT_PROTECTED 계약이 capture/route 경로 외에 cleanup 경로까지 일관되게 적용된다. 다만 본 journey 의 `ProtectedSourceNotificationDetector` 는 `category` (call/transport/navigation/alarm/progress) + MediaSession extras + MediaStyle template 까지 보는 반면, `TrayOrphanCleanupRunner` 는 `ActiveTrayEntry` projection 에 flags 만 노출하므로 cleanup 경로의 PROTECTED 신호는 더 좁다 (false-positive bias = skip too many; 음악/통화 cancel false-negative 가 더 위험하다는 Risks §R4 결정). 향후 false negative 가 관측되면 `ActiveTrayInspector` projection 을 확장해 본 journey 의 detector 를 cleanup 에서도 재사용한다. 관련 plan: `docs/plans/2026-04-28-fix-issue-524-tray-orphan-cleanup-button.md` (Tasks 1-3 shipped via PR #530; Tasks 4 + 6-7 via this PR; Task 5 ADB on R3CY2058DLJ deferred). 본 journey 의 Observable steps / Exit state / Verification recipe 는 변경 없음 — Code pointers cross-cut 만 추가.
