...
6a. **미분류 bulk-assign** (plan `2026-04-26-rules-bulk-assign-unassigned`) — 두 sub-section ("작업 필요" / "보류") 의 미분류 row 를 long-press 하면 multi-select 모드 진입 (해당 sub-section 의 한 bucket 안에서만 — cross-section 선택은 의도적으로 차단). 화면 상단에 `RulesMultiSelectActionBar` (`"${N}개 선택됨"` + "분류 추가" + "취소") 가 떠 있고, 같은 bucket 의 다른 row 탭은 toggle. "분류 추가" 탭 → `BulkRulesAssignBottomSheet` 노출 (기존 분류 N개 + 터미널 "+ 새 분류 만들기" 행). **Path A — 기존 분류**: 행 탭 → `BulkAssignRulesToCategoryUseCase.assign(ruleIds, categoryId)` 가 ruleId 별로 `appendRuleIdToCategory` → `markRuleAsAssigned` 를 ordered 호출 (단일 rule 경로와 동일 use case 위임) → 스낵바 `"규칙 N개를 '<카테고리명>' 분류에 추가했어요"`. **Path B — 새 분류**: 터미널 행 탭 → `CategoryEditorScreen` 이 `CategoryEditorPrefill(seedExistingRuleIds = selectedRuleIds, defaultAction = DIGEST)` 로 마운트 (defaultAction 은 conservative 기본 — 사용자가 dropdown 으로 PRIORITY 등으로 바꿀 수 있음). 저장 → editor 의 final `savedCategory.ruleIds` 를 동일 use case 로 전달 → 스낵바 `"새 분류 '<카테고리명>' 만들고 규칙 N개를 추가했어요"`. Editor cancel 은 multi-select 유지 (다른 분류 재시도 가능); ActionBar "취소" 탭만 selection clear.
...
- `domain/usecase/BulkAssignRulesToCategoryUseCase` — RulesScreen 의 multi-select bulk-assign 경로가 호출하는 thin wrapper. 입력 ruleIds 를 distinct 한 뒤 ruleId 별로 `AssignRuleToCategoryUseCase.assign` 위임. plan `2026-04-26-rules-bulk-assign-unassigned`
- `ui/screens/rules/RulesScreenMultiSelectState` — pure-Kotlin holder. `Bucket(ACTION_NEEDED, PARKED)` 별 `enterSelection` / `toggle` (cross-bucket toggle 무시) / 마지막 한 건 remove 시 auto-cancel
- `ui/screens/rules/RulesMultiSelectActionBar` — 선택 카운트 + "분류 추가" / "취소" CTA. LazyColumn 의 in-flow item 으로 마운트
- `ui/screens/rules/BulkRulesAssignBottomSheet` — bulk 경로 전용 sheet (단일 rule 경로의 `CategoryAssignSheetContent` 와 분리). 기존 Category 행 + 터미널 "새 분류 만들기" 행만 노출 (rule-specific 카피 + park CTA 없음)
- `domain/usecase/CategoryEditorPrefill` — `seedExistingRuleIds: List<String>` 필드 (additive, default empty) + nullable `pendingRule`. bulk Path B 가 seed list 를 채워 editor 를 띄움
...
- `ui/screens/rules/RulesUnassignedSection#UnassignedRuleRowSlot` — 두 sub-section 공용 row composable. parked 행에는 "작업으로 끌어올리기" TextButton 추가. plan `2026-04-27-refactor-rules-screen-split` 에서 `RulesScreen.kt` → `RulesUnassignedSection.kt` 로 carve-out
- `ui/screens/rules/RulesUnassignedSection#CategoryAssignSheetContent` — 저장 직후 ModalBottomSheet 의 분류 선택 본문 + 두 dismiss CTA ("작업 목록에 두기" / "분류 없이 보류"). 같은 plan 에서 carve-out
- `ui/screens/rules/RuleEditorOverrideSection` — override toggle / base dropdown / superset warning / `ruleRowPresentationFor` 헬퍼. plan `2026-04-27-refactor-rules-screen-split` 에서 분리
- `ui/screens/rules/RuleEditorTypeSection` — `RepeatBundleThresholdEditor`, `RuleEditorAppSuggestionRow`, `typeLabel`, `matchLabelFor`. 같은 plan 에서 분리
...
- `BulkAssignRulesToCategoryUseCaseTest` — bulk wrapper 의 4 케이스 (happy path ordered N회, distinct dedupe, empty short-circuit, mid-loop 실패 propagate)
- `RulesScreenMultiSelectStateTest` — bucket 별 `enterSelection` / `toggle` / cross-bucket 무시 / auto-cancel 계약
...
- (resolved 2026-04-27, plan `2026-04-27-refactor-rules-screen-split`) **code health — `RulesScreen.kt` 1446 lines** — sub-sections (unassigned slot + assign sheet, override editor, repeat-bundle / app-suggestion editor) carved into 3 sibling files (`RulesUnassignedSection.kt`, `RuleEditorOverrideSection.kt`, `RuleEditorTypeSection.kt`). Root file now 966 lines (33% reduction); plan's <=700 target not fully reached because the inline editor `AlertDialog` + `startCreate`/`startEdit` local funcs stay in root by Risks-section design. Behavior unchanged, characterization tests (PR #455) GREEN.
- 2026-04-28 DRIFT (journey-tester static review of PR #541 / commit `60ee856` against doc): Observable step 6 enumerates `RuleTypeUi: PERSON / APP / KEYWORD / SCHEDULE / REPEAT_BUNDLE` (5 types) but plan-526 Tasks 5-6 shipped a 6th value `SENDER` (`RuleUiModel.kt:62`) and exposed it in the editor type dropdown via the new hoisted `internal val RuleEditorTypeDropdownOptions: List<RuleTypeUi>` constant in `RuleEditorTypeSection.kt:137-144`. `RulesScreen.kt:655-661` now reads from that list (single source of truth, replacing the prior hard-coded 5-type list). Editor labels: SENDER → "발신자" type label, "발신자 이름" match label (`RuleEditorTypeSection.kt:156, 167`). Distinct from PERSON: substring + ignoreCase match against notification title to reach messenger DMs (Teams / Slack / KakaoTalk) where `sender` metadata is empty / set to the app name. Doc also missing reference to the companion Settings toggle "발신자 분류 제안" in `SettingsOperationalSection.kt:97-118` (default ON via `SmartNotiSettings.senderSuggestionEnabled = true`, `SettingsModels.kt:140`) which gates the Detail-screen one-tap promote-to-PRIORITY card path that the SENDER rule type powers. Both sites need an Observable-steps refresh + Code-pointers update. ADB end-to-end verification of the new dropdown row + Settings card not run this sweep — leaving `last-verified` at 2026-04-27. Plan: `docs/plans/2026-04-28-fix-issue-526-sender-aware-classification-rules.md` Tasks 5-6.
...
- 2026-04-27: ADB end-to-end re-verify on emulator-5554 (APK `versionName=0.1.0`, lastUpdateTime 11:56). PASS — Settings → 고급 섹션 "고급 규칙 편집 열기" 진입 (BottomNav 4탭 — "규칙" 탭 부재 재확인), baseline 활성 규칙 8개 (즉시 전달 5 / Digest 3 / 조용히 0; IGNORE chip 부재 — owning Category 없으므로 기대대로). "새 규칙 추가" → editor 다이얼로그가 `기본 정보 / 규칙 타입 / 예외 규칙` 3개 섹션만 노출 (구 "처리 방식" SectionLabel + RuleActionUi dropdown 부재 재확인 — D3 회귀 가드). 키워드 타입 + 이름 `VTest` + 매치값 `vtestkw` 로 저장 → 즉시 `CategoryAssignBottomSheet` 가 "이 규칙을 어떤 분류에 추가하시겠어요?" + 4개 기존 분류 (중요 알림 즉시 전달 / 프로모션 알림 Digest / 반복 알림 Digest / BulkNewCat Digest) + 두 dismiss CTA "분류 없이 보류" + "작업 목록에 두기" 노출. (Path B) "작업 목록에 두기" 탭 → 활성 규칙 8→9, 액션 그룹 위에 **"작업 필요"** SectionLabel + "분류에 추가되기 전까지 어떤 알림도 분류하지 않아요…" 배너 + VTest row (border-only "미분류" chip + "분류에 추가되기 전까지 비활성" 배너). (Path C) VTest 카드 탭 → sheet 재오픈 (4 Category + 2 dismiss CTA 동일) → "분류 없이 보류" 탭 → row 가 "작업 필요" → **"보류"** sub-section 으로 이동 ("사용자가 보류함 · 필요 시 작업으로 끌어올리세요" 배너 + "보류" chip + 우하단 "작업으로 끌어올리기" TextButton); 빈 "작업 필요" SectionLabel 사라짐 (regression guard PASS). (Path D) "작업으로 끌어올리기" 탭 → row 가 "보류" → "작업 필요" 로 승격, 빈 "보류" SectionLabel 사라짐 (regression guard PASS). 테스트 후 VTest 삭제로 baseline 복원 (활성 규칙 9→8). DRIFT 없음 — Observable steps 6 의 두-CTA sheet + 두 sub-section partition + promote/re-open 어포던스 모두 doc 과 일치. `last-verified` 2026-04-26 → 2026-04-27.
- 2026-04-26: **미분류 bulk-assign 도입** — RulesScreen 의 두 미분류 sub-section 에서 long-press → multi-select 모드 진입 (per-bucket, cross-section 차단), AppBar 자리에 `RulesMultiSelectActionBar` (선택 카운트 + "분류 추가" + "취소"), "분류 추가" 탭 → `BulkRulesAssignBottomSheet` (기존 분류 N개 + "+ 새 분류 만들기" 행). Path A 는 신규 `BulkAssignRulesToCategoryUseCase` 가 selected ruleId 별로 단일 use case (`AssignRuleToCategoryUseCase.assign`) 에 위임 — distinct dedup + ordered append→flip 계약 N회 반복. Path B 는 `CategoryEditorPrefill(seedExistingRuleIds=...)` 로 `CategoryEditorScreen` 띄움 (defaultAction = DIGEST conservative default — bulk 케이스에는 dynamic-opposite 의 source action 이 없음). 저장 직후 editor 의 final `savedCategory.ruleIds` 를 동일 use case 로 전달 — 사용자가 picker 에서 un-toggle 한 seed 는 존중. 두 경로 모두 snackbar 로 confirm. CategoryEditorPrefill 에 nullable `pendingRule` + `seedExistingRuleIds: List<String>` 필드 (additive — Detail 단일 rule 경로 호출부 무변경). 신규 단위 테스트 (Tasks 1-2): `BulkAssignRulesToCategoryUseCaseTest` (4 케이스 — happy path ordered N회, distinct dedupe, empty short-circuit, mid-loop 실패 propagate). 신규 pure state (Tasks 3-4): `RulesScreenMultiSelectStateTest` (bucket 별 enter/toggle/cross-bucket 무시/auto-cancel 4 케이스). Plan: `docs/plans/2026-04-26-rules-bulk-assign-unassigned.md` Tasks 1-7 (#382 `92ecf0d`, #383 `0f6d0b2`, this PR `c34baf5`). ADB end-to-end re-verify on emulator-5554 (APK rebuilt 2026-04-26 22:55 KST) PASS — (a) Settings → 고급 규칙 편집, baseline `활성 규칙 4개 · 즉시 전달 2 · Digest 2`. (b) `새 규칙 추가` 키워드 룰 3개 (`BulkAsgnTest1/2/3`, matchValue `bulkasgnkw1/2/3`) → 각각 sheet 의 "작업 목록에 두기" 로 dismiss → "활성 규칙 7개" + "작업 필요" sub-section 에 3 row. (c) BulkAsgnTest1 long-press → ActionBar `1개 선택됨` + `취소` + `분류 추가` 노출. BulkAsgnTest2 탭 → `2개 선택됨` 으로 갱신. (d) Path A: `분류 추가` → `BulkRulesAssignBottomSheet` ("선택한 규칙 2개를 어디에 추가할까요?" + 3 기존 분류 + "+ 새 분류 만들기") → `중요 알림` 행 탭 → 즉시 전달 2 → 4 (BulkAsgnTest1/2 합류). (e) Path B: 남은 BulkAsgnTest3 long-press → `1개 선택됨` → `분류 추가` → bulk sheet 가 중요 알림 `규칙 4개` 로 업데이트된 상태로 노출 → `+ 새 분류 만들기` 탭 → `CategoryEditorScreen` 가 `전달 방식 = Digest` (DIGEST default 확인) + `소속 규칙` chip picker 에 BulkAsgnTest3 pre-selected 로 마운트 → 이름 `BulkNewCat` 입력 → `추가` 탭 → 스낵바 `"새 분류 'BulkNewCat' 만들고 규칙 1개를 추가했어요"` 표시 + Categories 탭에 `BulkNewCat` 신규 entry 확인. DRIFT 없음 — Path A/B 모두 doc 의 step 6a 와 일치. `last-verified` 변경 없음 (2026-04-26 유지).
- 2026-04-27: **`RulesScreen.kt` per-section split** — 1446-line single file 을 같은 패키지의 sub-file 3개로 carve out: `RulesUnassignedSection.kt` (`UnassignedRuleRowSlot` + `CategoryAssignSheetContent` + `categoryActionLabel`, ~247 lines), `RuleEditorOverrideSection.kt` (`RuleOverrideEditorSection` + `RuleOverrideBaseDropdown` + `supersetWarningMessage` + `ruleRowPresentationFor`, ~158 lines), `RuleEditorTypeSection.kt` (`RepeatBundleThresholdEditor` + `RuleEditorAppSuggestionRow` + `typeLabel` + `matchLabelFor`, ~142 lines). 모든 carve-out 함수는 `private → internal` 로 승격. Root `RulesScreen.kt` 는 1446 → 966 lines (33% 감소) — plan 의 `≤700` 목표는 inline editor `AlertDialog` + `startCreate`/`startEdit` 로컬 펑션이 root 에 남기로 한 Risks-section 설계상 미달성. 동작 변경 없음 — Task 1 의 12개 characterization tests (PR #455) GREEN 유지. Plan: `docs/plans/2026-04-27-refactor-rules-screen-split.md` Tasks 2-4 (this PR). `last-verified` 변경 없음 (refactor — observable behavior 변경 없음).
