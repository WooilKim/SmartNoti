...
- 2026-04-27: **Re-verification PASS (post-#491 PROMO_QUIETING action SILENT→DIGEST + #485 DiagnosticLogger + #492 ContentSignatureNormalizer)** — emulator-5554, APK rebuild `lastUpdateTime=2026-04-28 02:00:47` (commit range includes `64b1382` PR #491). Recipe 5단계 모두 통과 (헤더 `보관 11건 · 처리 12건` + 기본 탭 `보관 중 · 11건` 선택, 처리됨 탭 tap 후 서브카피 `이미 확인했거나` 1건, 두 탭 collapsed default, Shell 그룹 헤더 tap → expand 후 `최근 묶음 미리보기` + `전체 보기 · 8건 더` CTA + `모두 중요로 복구` / `모두 지우기` bulk action row). Inbox 3-탭 구조 (정리함 / 보관 중 / 처리됨) 무회귀. PR #491 의 PROMO_QUIETING migration 동작 확인: `smartnoti_categories.preferences_pb` 의 `cat-onboarding-promo_quieting` 행 action 필드 = `DIGEST` (이전 `SILENT`), `smartnoti_settings` 의 `promo_quieting_action_migration_v3_applied` 플래그 set. Hidden 탭 신규 PROMO 경로 차단 — 기존 SILENT 행은 그대로 보관/처리 두 탭에 남고, 신규 PROMO 분류는 [digest-inbox](digest-inbox.md) 로 라우팅 (즉, Hidden 카운트는 신규 PROMO 유입 0건). DB schema version 9 유지 — PR #491 은 Room migration 이 아닌 Categories preferences pb 의 onboarding row migration. `last-verified` 는 same-day 변경 없음 (이미 2026-04-27).
