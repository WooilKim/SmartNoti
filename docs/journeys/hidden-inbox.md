...
2. `NotificationRepository.getInstance(context).observeAll()` 을 `collectAsStateWithLifecycle` 로 구독.
3. `allNotifications.filter { it.status == NotificationStatusUi.SILENT }` 로 목록 도출.
4. 상단 영역:
...
5. 비어 있으면 `EmptyState("아직 숨긴 알림이 없어요", "조용히 처리된 알림이 생기면 여기에 모여요")`.
6. 있으면 `SmartSurfaceCard` + `NotificationCard` 리스트 (`key = it.id`).
7. 카드 탭 → `navController.navigate(Routes.Detail.create(id))` (→ [notification-detail](notification-detail.md)).
8. 뒤로가기 → `navController.popBackStack()`.
