...
- **2026-04-22 DRIFT (journey-tester, emulator-5554, fresh Categories APK)**: Path B ("새 분류 만들기") 경로에서 `CategoryEditorScreen` 의 "추가" 버튼을 탭해도 **신규 Category 가 persist 되지 않는다**. 재현: fresh install 에 기존 Category 0 건 상태에서 AssignTest 알림을 post → Detail 의 "분류 변경" → 시트에는 "+ 새 분류 만들기" terminal row 만 노출 (기대한 대로 기존 분류 없음) → editor 가 prefill 된 name/action/rule 로 AlertDialog 형태로 열림 → "추가" 탭. 결과: 분류 탭이 여전히 "아직 분류가 없어요" 상태로 유지되고 `files/datastore/` 에 `smartnoti_categories.preferences_pb` 파일이 생성되지 않는다 (기존 `smartnoti_rules.preferences_pb` / `smartnoti_settings.preferences_pb` 두 파일만 존재). 기대: Exit state "신규 Category 가 저장되며 ruleIds 에 rule.id 포함". Observable 3.3-3.4 와 불일치. 후속 sender 가 여전히 SILENT fallback 으로 떨어지는지 여부는 이 precondition 미충족으로 측정 불가.
