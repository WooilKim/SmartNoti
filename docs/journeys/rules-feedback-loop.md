...
- (resolved 2026-04-25, plan 2026-04-24-detail-reclassify-confirm-toast) 시트 확정 후 사용자에게 토스트나 확인 UI 없음 (조용히 성공). → plan: `docs/plans/2026-04-24-detail-reclassify-confirm-toast.md`
...
- 2026-04-27: journey-tester re-verify on emulator-5554 — Path A end-to-end PASS on shell-posted `DetailTest_0427` (sender 미부여 → derivation rule type = APP). 정리함 > 보관 중 > Shell 카드 (12건) > preview 탭 → Detail `분류` "조용히 정리". 하단 "분류 변경" 탭 → `CategoryAssignBottomSheet` 가 onboarding-seeded 3개 + 사용자 `BulkNewCat` 합쳐 4개 Category + "+ 새 분류 만들기" terminal row 로 렌더 (action chip "즉시 전달" / "Digest" / "Digest" / "Digest"). "중요 알림" 탭 → 시트 dismiss → Detail 의 `분류` 가 `즉시 전달` 로 즉시 갱신, reasonTag `사용자 분류` append (2026-04-26 detail-reclassify-this-row-now), `"중요 알림 분류로 옮겼어요"` 스낵바 표시 (2026-04-25 confirm-toast). 자동 재분류: 동일 Shell pkg 후속 알림 게시 직후 Home StatPill `즉시 22 → 24` (+2 = Path A row 이동 +1 + 새 알림 +1), `조용히 25 → 24` (-1 = Path A row 이동) — APP:com.android.shell rule 기 존재로 dedup-append no-op (사전 baseline) + classifier cascade 가 PRIORITY 적용 즉시 검증. `last-verified` 2026-04-26 → 2026-04-27.
