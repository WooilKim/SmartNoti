...
- `ui/components/DigestGroupCard` — `PREVIEW_LIMIT` (3건) + `showAll` `rememberSaveable` 토글 + inline "전체 보기 · ${remaining}건 더" CTA (그룹 안 모든 row 의 Detail 진입 경로). bulkActions 는 CTA 아래에 그대로 렌더되어 그룹 전체 (DB 의 모든 row) 를 대상으로 동작 — `showAll` 상태와 무관. Preview 행은 `NotificationCard(embeddedInCard = true)` 로 호출되어 자체 outlined surface 가 사라지고 (F1) 행 사이는 1dp `BorderSubtle` `HorizontalDivider` 로만 구분 — 부모 group card 가 단일 카드 경계가 됨.
- `ui/components/NotificationCard` — `embeddedInCard: Boolean = false` 옵트아웃 파라미터로 outlined card surface 를 제어 (F1). `false` (default) → 기존대로 status-tinted Card + 1dp border 렌더 (Home recent feed / IgnoredArchive / Detail-related list / PriorityScreen). `true` → flat row content (Card 부재, fill 부재, border 부재) — 호출자가 divider rhythm 책임. 가시성 결정은 pure helper `notificationCardSurfaceVisible(embeddedInCard)` 로 추출되어 `NotificationCardSurfaceVisibilityTest` 가 contract pin.
...
- `NotificationCardSurfaceVisibilityTest` — F1 의 outlined-surface 가시성 계약 (`notificationCardSurfaceVisible(embeddedInCard)`) 을 pin: `false` (default) → surface 렌더 (standalone), `true` → suppress (DigestGroupCard preview row). 회귀 PR 이 default 를 뒤집거나 embedded 모드에서 surface 를 다시 렌더하면 즉시 fail.
...
- **2026-04-28 ui-ux sweep (MODERATE × 5)** — 사용자 정성 피드백 "정리함이 정돈된 느낌이 전혀 없어" 에 대응한 audit (emulator-5554) 에서 5개 시각 부채 발견: ~~(F2) ScreenHeader + subtitle + sort dropdown row + InboxTabRow 가 첫 화면 25% 를 chrome 으로 점유~~ — **shipped 2026-04-28 (Change log 참고, PR #506-cohort F2)**, ~~(F3) 같은 화면에 orange accent 가 6회 등장 (group `Nn건` badge × 2 + 행별 `Digest` chip × 4) — accent 절제 원칙 위반~~ → **shipped 2026-04-28 (Change log 참고, PR #506-cohort F3)**, ~~(F1) Digest 서브탭의 `DigestGroupCard` 가 카드 안에 카드를 중첩 (preview row 도 자체 outlined surface)~~ → **shipped 2026-04-28 (Change log 참고, PR #506-cohort F1)**. 잔여 2건: (F4) 한 화면 안에 4종의 카드 시각 언어 (summary / group / preview / nav) 가 corner radius / border / padding 모두 다름, (F5) 보관 중 / 처리됨 의 summary 카드가 탭 라벨과 그룹 카드와 카운트를 3중으로 반복. 후속: 사용자 우선순위 (`f2 부터 차례대로`) 에 따라 F5 → F4 순으로 별도 PR 분기.
...
- 2026-04-28: Meta-plan `2026-04-28-meta-inbox-organized-feel-overhaul.md` finding **F1** (cards-inside-cards collapse) shipped — `NotificationCard` 에 opt-out `embeddedInCard: Boolean = false` 파라미터 추가. `DigestGroupCard` preview row 가 `embeddedInCard = true` 로 호출되면 outlined card surface (rounded outer Card + status-tinted fill + 1dp border) 를 모두 suppress 하고 flat row content (`Column { Row, Title, Body, ReasonChips }`) 만 렌더. 행과 행 사이는 1dp `BorderSubtle` `HorizontalDivider` 로 분리되어 부모 group card 가 단일 카드 경계가 됨 (이전: outer Shell card 안에 3개의 inner outlined NotificationCard — 600px 안에 3-layer 중첩 사각형 → 부모 card 안에 divider-separated rows). Standalone NotificationCard 호출 (Home recent feed / IgnoredArchive / Detail-related list / PriorityScreen) 은 default `false` 로 기존 outlined surface 를 그대로 유지 — cross-status feed 의 per-row card affordance 보존. 가시성 결정은 pure helper `notificationCardSurfaceVisible(embeddedInCard)` 로 추출되어 `NotificationCardSurfaceVisibilityTest` (2 case) 가 contract pin (codebase 의 `notificationCardStatusBadgeVisible` / `digestGroupCardPreviewState` pattern mirror). 행동 / 인터랙션 변화 없음 — `clickModifier` (tap → Detail + PriorityScreen 의 long-press 멀티셀렉트) 는 Card 분기와 flat 분기 모두 동일하게 attach. ADB 검증 on emulator-5554: 정리함 진입 후 Shell 그룹 (3건) preview 영역 uiautomator dump → 3 row (`광고 / Shopping / Shopping`) 모두 동일 left-edge x=84 (parent card padding 일치, inset 부재), row 사이에 thin divider 만 노출, inner Card border 노드 부재. Bulk action row (`모두 중요로 변경` / `모두 지우기`) 는 변동 없음. Standalone Home 탭 회귀 없음 — recent notification card 가 기존대로 outlined surface 로 렌더됨. 회귀 가드: 신규 `NotificationCardSurfaceVisibilityTest` 2 case + 기존 `NotificationCardStatusBadgeVisibilityTest` / `DigestGroupCardSeeAllTest` / `DigestGroupHeaderBadgeStyleTest` 모두 pass + full unit suite + `:app:assembleDebug` green. `last-verified` 는 verification recipe 전체를 재실행하지 않았으므로 갱신 보류 (per `.claude/rules/docs-sync.md`).
- 2026-04-28: Meta-plan `2026-04-28-meta-inbox-organized-feel-overhaul.md` finding **F3** (orange accent restraint) shipped — `DigestGroupCard` 헤더의 `${count}건` 카운트 배지를 filled-orange pill (DigestContainer/DigestOnContainer) → outline-neutral chip (BorderSubtle border + onSurfaceVariant text) 로 demote (Hidden 의 archive/processed 그룹도 같은 primitive 라 함께 정돈됨). `NotificationCard` 에 opt-out `showStatusBadge` 파라미터 추가 (default `true`) — `DigestGroupCard` preview row 호출 시 `false` 를 넘겨 행별 `Digest` 오렌지 chip 을 suppress (Home recent feed / IgnoredArchive / Detail-related list / PriorityScreen 등 standalone 호출자는 default 로 chip 유지). 결과: inbox-unified Digest 서브탭 첫 화면의 orange accent 빈도가 6회 → 사실상 0회 (선택된 sub-tab segment 의 primary blue tint 가 selection signal 로 단독 점유). 두 시각 결정은 pure helper (`digestGroupHeaderBadgeStyle`, `notificationCardStatusBadgeVisible`) 로 추출되어 `DigestGroupHeaderBadgeStyleTest` + `NotificationCardStatusBadgeVisibilityTest` 가 contract pin (codebase 의 `digestGroupCardPreviewState` pattern mirror). ADB 검증 on emulator-5554: 정리함 진입 후 uiautomator dump 에서 `text="Digest"` chip 노드 0건 (이전: preview row 마다 1건), Shell 그룹 `3건` 배지 outline-neutral 렌더 (filled bg 부재), `보관 중` 탭 전환 후 Shell `11건` 배지도 동일 정돈 확인. 회귀 없음 가드: `DigestGroupCardSeeAllTest` 8 case + 신규 두 contract test pass + full unit suite + `:app:assembleDebug` green. `last-verified` 는 verification recipe 전체를 재실행하지 않았으므로 갱신 보류 (per `.claude/rules/docs-sync.md`).
