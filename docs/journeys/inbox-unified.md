...
- `ui/screens/digest/DigestScreen` — Digest 서브탭 본체 (재사용). `groups.isEmpty()` 분기에서 [DigestEmptyStateAction] 의 카피 + `EmptyState` 의 `action` 슬롯으로 inline `숨길 앱 선택하기` CTA 노출 (호스트가 plumb 하는 `onOpenSuppressedAppsSettings` lambda 로 Settings 진입).
