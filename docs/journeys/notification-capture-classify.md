...
- **doc drift (2026-04-28, journey-tester sweep against PR #536 / commit `bf00af0`)**: Observable Step 8 still enumerates the rule-type set as `(PERSON / APP / KEYWORD / SCHEDULE / REPEAT_BUNDLE)` and the rule-type ladder as `(APP > KEYWORD > PERSON > SCHEDULE > REPEAT_BUNDLE)`, but PR #536 (issue #526 Tasks 1-2) shipped a 6th case `RuleTypeUi.SENDER` (`RuleUiModel.kt:62`) and reordered the `CategoryConflictResolver` ladder to `APP > SENDER > KEYWORD > PERSON > SCHEDULE > REPEAT_BUNDLE` (`CategoryConflictResolver.kt:108-126`). SENDER matches `input.title` substring + ignoreCase (`NotificationClassifier.kt:189-190`) and is gated by `matchValue.isNotBlank()`. Plan `docs/plans/2026-04-28-fix-issue-526-sender-aware-classification-rules.md` Task 9 (journey docs sync) is still planned/unstarted — this drift is expected and will be resolved when Tasks 8-9 ship. No `last-verified` bump (recipe not re-run on hardware; test gate `NotificationClassifierSenderRuleTest` already ships green via PR #536).
