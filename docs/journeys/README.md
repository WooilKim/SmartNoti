...
| hidden-inbox | PASS | UTC 17:01Z re-verification on emulator-5554 after rebuilding APK to include PR #491 PROMO_QUIETING migration (`lastUpdateTime=2026-04-28 02:00:47`; commit range covers `64b1382` PR #491 + `c6d9451` PR #489, excludes #492 ContentSignatureNormalizer/#485 DiagnosticLogger which are default-OFF anyway). All 5 recipe steps pass on `Routes.Hidden` cold-start: 헤더 `보관 11건 · 처리 12건` + 기본 `보관 중 · 11건` 탭, `처리됨 · 12건` 탭 tap (786,580) → 서브카피 `이미 확인했거나` 1건, 두 탭 collapsed default (`최근 묶음 미리보기` 0건), Shell 그룹 헤더 tap → expand 후 `최근 묶음 미리보기` + 3 preview rows + `전체 보기 · 8건 더` CTA + `모두 중요로 복구` / `모두 지우기` bulk action row. **Bug B2 (PR #491) 회귀 없음**: `smartnoti_categories.preferences_pb` 의 `cat-onboarding-promo_quieting` 행 action 필드 `DIGEST` (이전 `SILENT`), `smartnoti_settings.preferences_pb` 의 `promo_quieting_action_migration_v3_applied` 플래그 set — 신규 PROMO 분류는 [digest-inbox](digest-inbox.md) 로 라우팅되어 Hidden 탭에 더 이상 추가되지 않음. 기존 SILENT 행 (Shell · 11건) 은 그대로 보관/처리 두 탭에 잔존. Inbox 3-탭 (정리함 / 보관 중 / 처리됨) 구조 무회귀. DB schema version 9 유지 — PR #491 은 Room migration 이 아닌 Categories preferences pb 의 onboarding row migration. `last-verified` same-day (2026-04-27) — bump 없음. |
...
### 2026-04-27 (journey-tester — home-uncategorized-prompt post-#430 re-sweep, emulator-5554)

| Journey | Result | Notes |
|---|---|---|
| home-uncategorized-prompt | SKIP | Re-attempt at UTC 05:54Z (caller noted snooze should be past — actual snooze epoch unchanged at 1777274058159 = 07:14:18Z, still ~1h20m forward). Live UI launch (`am start … MainActivity` + `uiautomator dump`) found no `분류 제안 / 새 앱 / 분류 만들기 / 나중에` strings as expected (snooze still active). Cross-checked datastore: APP rule `APP%3Acom.android.shell` exists in `smartnoti_rules.preferences_pb` AND is referenced by `cat-onboarding-important_priority.ruleIds` (encoded `…%2CAPP%253Acom.android.shell`) — meaning the new #430 detector path treats `com.android.shell` as covered (excluded from prompt) on this device. Contract re-checked: `:app:testDebugUnitTest --tests UncategorizedAppsDetectorTest --rerun-tasks` 12/12 GREEN (host JBR). Live visual confirmation of APP-Rule coverage filter still requires snooze expire (~07:14Z) or destructive `pm clear`. `last-verified` 미bump (2026-04-26 유지). |
...
| Journey | Result | Notes |
|---|---|---|
| onboarding-bootstrap | SKIP | A.1 rotation pick (oldest `last-verified` 2026-04-24 vs other 18 at 2026-04-26, gap ≥ 1 day). Recipe step 1 is `adb shell pm clear com.smartnoti.app` which is destructive per `journey-tester` agent safety rules — wipes all app state + NotificationListener permission grant. Skipped without explicit caller confirmation. Recipe itself flags the warning at line 85. `last-verified` unchanged. Suggested: run manually next session (sweep schedule allows), or carve out a non-destructive sub-recipe (e.g. inspect already-bootstrapped tray on a freshly-onboarded device) for autonomous verification. |
