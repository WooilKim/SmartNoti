...
5. `SmartNotiNotifier.notifySuppressedNotification(DIGEST, ...)` 호출:
   ...
   - **Visual identity** (PR #521, #510): smallIcon = `R.drawable.ic_replacement_digest` (inbox 글리프, "DIGEST" 액션 식별), largeIcon = `AppIconResolver.resolve(sourcePackageName)` 결과 (source 앱 launcher 아이콘). Resolver 가 null 을 반환하면 (system service / disabled / no launcher icon) largeIcon 을 omit — SmartNoti 브랜드로 채우지 않는다. PRIORITY pass-through 분기는 smallIcon = `R.drawable.ic_replacement_priority` (notifications 글리프) 로 동일 메커니즘.
...
- `data/local/HighVolumeAppDetector` + `data/local/HighVolumeAppCandidate` (Issue #525, PR #532) — 정리함 Digest 서브탭의 inline suggestion card 가 호출하는 7-day rolling detector. 사용자가 `[예, 묶을게요]` 를 누르면 `SettingsRepository.setSuppressedSourceApps(current + pkg)` 가 본 journey 의 `currentApps` 를 명시적으로 확장 — 자동 expansion 과 동일 endpoint 를 공유하므로 이후 Observable steps 1-7 의 contract 가 그대로 적용된다 (단, 사용자가 카드를 통해 의지를 표현했으므로 `SuppressedSourceAppsAutoExpansionPolicy` 의 sticky-exclude / 빈 set no-op 가드와는 독립적인 경로). 자세한 카드 surface 는 → [inbox-unified](inbox-unified.md) Observable step 7.
...
- `notification/SmartNotiNotifier#notifySuppressedNotification` — replacement 빌더. **#510 (PR #521)**: smallIcon = `ReplacementActionIcon.{DIGEST,PRIORITY}.drawableRes` (decision-driven), largeIcon = `appIconResolver.resolve(sourcePackageName)` (null 이면 omit).
- `notification/AppIconResolver` (PR #521) — PackageManager → Drawable → Bitmap conversion + `LruCache<String, Bitmap>(64)` 메모이즈 + install/upgrade/remove 인밸리데이션. Returns null when the source app exposes no displayable launcher icon. Plan: `docs/plans/2026-04-27-fix-issue-510-replacement-icon-source-action-overlay.md`.
- `notification/ReplacementActionIcon` (PR #521) — compile-time enum: DIGEST → `R.drawable.ic_replacement_digest`, PRIORITY → `R.drawable.ic_replacement_priority`. Same plan.
...
- 2026-04-27: ADB verification PASS (emulator-5554) — testnotifier `PROMO_DIGEST` 시나리오 tap 후 `com.smartnoti.testnotifier` 의 "오늘만 특가 안내" payload 가 active list 에서 사라지고 동일 title 이 `com.smartnoti.app` 의 `smartnoti_replacement_digest_default_light_private_noheadsup` 채널 (importance=3, flags=0x18 AutoCancel) 에 새 entry 로 게시됨. dumpsys 의 `timeout=1970-01-01 00:30:00` 으로 신규 30분 auto-dismiss (PR #417, `setTimeoutAfter`) 가 실제 발화 가능한 형태로 활성. Observable steps 1–5 + Exit state 일치, DRIFT 없음.
- 2026-04-27: **#511 fix — DIGEST 분기도 단일 invariant 로 통일** (PR #513 `5db1208` + closure PR #517). `SourceNotificationRoutingPolicy.route(DIGEST, ...)` 가 이제 suppress-list 멤버십과 무관하게 항상 `cancelSourceNotification = true AND notifyReplacementNotification = true` 를 반환한다 (Option C cancel-after-post invariant). 이전엔 `NotificationSuppressionPolicy.shouldSuppressSourceNotification(...)` 의 boolean 결과를 통해 conditional 이었고, 2026-04-24 default-on 변경 (`suppressSourceForDigestAndSilent = true` + 빈 set = all-apps) 으로 사실상 모든 사용자가 suppression 분기로 흘렀지만, suppress-list 의미가 분기 결정에 직접 영향을 주는 구조 자체는 남아 있었다. Option C 이후엔 suppress-list 가 더 이상 cancel/replace 결정의 게이트가 아니다 — 단순히 "다음 번 자동 추가 여부" 만을 기록하는 의미로 축소된다 (`SuppressedSourceAppsAutoExpansionPolicy` 는 그대로 동작). Observable steps 1-2 의 wording 은 그대로 유효 (auto-expansion + suppress 결과는 동일) 이지만, step 3 의 `route(...)` 는 입력 `suppress` 파라미터에 무관하게 동일한 결과를 돌려준다는 점만 갱신. Verification recipe (testnotifier `PROMO_DIGEST` tap 후 dumpsys grep) 은 변경 없음. PRIORITY pass-through 분기는 `notifyReplacementNotification = false` 그대로 (Out of scope 보장). Migration runner (`MigrateOrphanedSourceCancellationRunner`) 가 1회성으로 cohort 정리 — DataStore flag `migrate_orphaned_source_cancellation_v1_applied`. `last-verified` 2026-04-27 → 2026-04-27 unchanged (same day; verification recipe 는 본 변경 후에도 그대로 PASS, 트레이 중복 추가 검증은 silent-auto-hide journey 의 R3CY2058DLJ 라이브 캡처가 cross-cuts).
...
- 2026-04-27: **Issue #510 closure — DIGEST replacement notifications carry source-app large icon + DIGEST action small icon glyph**. `SmartNotiNotifier#notifySuppressedNotification` 의 DIGEST 분기에서 smallIcon = `R.drawable.ic_replacement_digest` (inbox 글리프), largeIcon = `AppIconResolver.resolve(sourcePackageName)` (null 이면 omit) 이 적용된다. PRIORITY 분기는 smallIcon = `R.drawable.ic_replacement_priority` (notifications 글리프) 로 동일 메커니즘. 기존 정적 `android.R.drawable.ic_dialog_info` smallIcon + 미설정 largeIcon → DIGEST/PRIORITY/SILENT replacement 가 트레이에서 시각적으로 구분 불가능했던 회귀 차단. Tasks 1-4 shipped via PR #521 (commit `cfe60e5`); Tasks 5-7 (ADB e2e + journey bumps + plan flip status:shipped) shipped via this PR. 본 journey 는 SILENT 채널을 직접 사용하지 않으므로 R3CY2058DLJ 라이브 e2e 는 silent-auto-hide journey 의 동일 빌드 capture 가 cross-cuts (`smartnoti_silent_*` channels 의 `id=0x7f070099` = `ic_replacement_silent` + Gmail child 의 `android.largeIcon=Icon(typ=BITMAP size=101x101)` resolved through `AppIconResolver`); DIGEST 분기는 동일 builder helper 를 공유하므로 contract 가 동시에 검증된다. Plan: `docs/plans/2026-04-27-fix-issue-510-replacement-icon-source-action-overlay.md` flipped to `status:shipped`. `last-verified` 2026-04-27 unchanged (same-day verification).
- 2026-04-28: **Issue #525 closure — 사용자 의지 기반 명시적 suppress 경로 추가** (plan `docs/plans/2026-04-28-fix-issue-525-high-volume-app-suggest-suppression.md` Tasks 1-9 shipped via PR #532 + 본 PR). 정리함 Digest 서브탭에 `InboxSuggestionCard` 가 노출되고 사용자가 `[예, 묶을게요]` 를 누르면 `SettingsRepository.setSuppressedSourceApps(current + pkg)` 가 본 journey 의 `currentApps` 를 명시적으로 확장한다 — 이후 Observable steps 1-7 의 contract 그대로 적용 (다음 DIGEST 알림이 들어오면 cancel + replacement 게시). 자동 expansion (`SuppressedSourceAppsAutoExpansionPolicy`) 과 endpoint 를 공유하므로 본 journey 의 `Observable steps` wording 변화 없음 — Code pointers 에 cross-cut 추가만. `HighVolumeAppDetector` 의 7-day rolling 쿼리는 PRIORITY-only 패키지를 후보에서 제외하므로 사용자가 이미 PRIORITY 로 분류한 앱 (친구 메신저 / 가족 connection) 이 카드로 권유되지 않는다. 카드 surface / 3 button (`예, 묶을게요` / `나중에` / `무시`) / 24h snooze / sticky dismiss 등 사용자 관측 UI 는 → [inbox-unified](inbox-unified.md) Observable step 7. `last-verified` 는 본 plan 이 본 journey 의 verification recipe (testnotifier `PROMO_DIGEST` tap → dumpsys grep) 를 영향주지 않으므로 갱신 보류 (per `.claude/rules/docs-sync.md`).
- 2026-04-28: **Issue #524 — Settings → "트레이 정리" 카드는 DIGEST cohort pre-#511 잔존도 정리한다**. `TrayOrphanCleanupRunner` 의 식별 알고리즘은 `smartnoti_silent_group_app:<pkg>` group key 기반이라 본 journey 의 핵심 분기인 `SmartNotiNotifier#notifySuppressedNotification` (DIGEST 채널 `smartnoti_replacement_digest_*`) 가 직접 산출하는 group key 와는 다르지만, #511 fix 가 들어오기 전에 게시되어 `replacementNotificationIssued = 0` 으로 저장된 cohort 중 SILENT/DIGEST 양 분기 모두 source 가 트레이에 잔존하던 경우, SmartNoti 의 silent_group 그룹과 같은 source packageName 에 속한 비-SmartNoti entry 는 cleanup 대상이 된다 (단일 pass O(N)). PERSISTENT_PROTECTED (`FLAG_FOREGROUND_SERVICE | FLAG_NO_CLEAR | FLAG_ONGOING_EVENT`) skip 계약은 본 journey 의 protected-source 약속과 일치 (cross-cut). 사용자 trigger 만; 자동 sweep 아님. Code pointers cross-cut: `data/local/TrayOrphanCleanupRunner`, `data/local/ActiveTrayInspector`, `ui/screens/settings/SettingsTrayCleanupSection`. 관련 plan: `docs/plans/2026-04-28-fix-issue-524-tray-orphan-cleanup-button.md` (Tasks 1-3 shipped via PR #530, Tasks 4 + 6-7 via this PR; Task 5 ADB on R3CY2058DLJ deferred — recipe preserved in plan §Task 5).
