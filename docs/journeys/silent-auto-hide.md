...
3. `NotificationRepository.save(...)` 로 DB 에 `status = SILENT` row 저장. listener 는 신규 SILENT × non-protected × non-persistent 캡처에 대해 `silentMode = SilentMode.ARCHIVED` 를 주입하므로, 신규 row 는 기본적으로 ARCHIVED 로 저장되어 "보관 중" 탭에 곧장 쌓인다. PROTECTED / PERSISTENT 경로는 기존대로 legacy (`silentMode = null`) 저장을 유지. capture 시점에 `StatusBarNotification#key` 도 `sourceEntryKey` 컬럼으로 함께 저장되어 이후 Detail 의 tray cancel 체인에 사용된다.
...
9. "처리 완료로 표시" 탭 → `NotificationRepository.markSilentProcessed(id)` 가 해당 row 의 `silentMode` 를 `ARCHIVED → PROCESSED` 로 upsert. `reasonTags` 에 `사용자 처리` 가 추가된다. (이미 PROCESSED 거나 SILENT 가 아니면 no-op false 반환.) DB flip 이 성공하면 Detail 이 `NotificationRepository.sourceEntryKeyForId(id)` 로 저장된 `sourceEntryKey` 를 조회하고 `SmartNotiNotificationListenerService.cancelSourceEntryIfConnected(key)` 를 호출해 tray 원본까지 제거한다. live listener 인스턴스가 없거나 `sourceEntryKey` 가 null (legacy row) 이면 tray cancel 은 best-effort 로 no-op — DB 전이는 그대로 보존된다.
...
# 2. 신규 SILENT 는 ARCHIVED 로 저장되어 tray 원본이 남고 요약 알림이 게시됨
adb shell dumpsys notification --noredact | grep -A2 smartnoti_silent_summary
adb shell dumpsys notification --noredact | grep Promo1
# expected: 원본 Promo1 알림이 tray 에 그대로, 요약 채널 smartnoti_silent_summary 에 "숨겨진 알림 1건"
...
# expected: 기본 "보관 중" 탭에 Promo1 카드 표시

# 4. Detail → "처리 완료로 표시" 가 DB 전이 + tray cancel 을 체인으로 수행
#    카드 탭 → Detail → "처리 완료로 표시" 버튼
adb shell dumpsys notification --noredact | grep Promo1
# expected: 원본 Promo1 이 tray 에서 사라짐. Hidden "처리됨" 탭에 row 이동.
```
...
- **Live listener 부재 시 tray cancel 불가** — Detail "처리 완료" 가 DB flip 에는 성공해도, listener service 가 disconnect 된 상태라면 `cancelSourceEntryIfConnected` 가 false 를 반환해 tray 원본은 남는다. 의도된 best-effort 동작이지만, 사용자는 "처리했는데 tray 에 아직 있네?" 로 오해할 수 있음. (plan Risks 에 기록된 fall-through.)
...
- 2026-04-20: **Capture 경로의 ARCHIVED 기본값 배선 완료** — `SmartNotiNotificationListenerService` 가 SILENT × non-protected × non-persistent 캡처에 대해 `SourceNotificationRoutingPolicy.route(..., silentMode = SilentMode.ARCHIVED)` 를 호출하고 저장 시점에 ARCHIVED + `sourceEntryKey` 를 함께 영속화. 신규 SILENT 가 기본적으로 "보관 중" 탭에 쌓이게 됨. 이전 Known gap "Capture 경로의 ARCHIVED 기본값 미배선" 해소. 관련 plan: `docs/plans/2026-04-20-silent-archive-drift-fix.md` Task 1–2, PR #125.
- 2026-04-20: **Detail "처리 완료" → tray cancel 체인 완료** — `NotificationRepository.sourceEntryKeyForId(id)` + `SmartNotiNotificationListenerService.cancelSourceEntryIfConnected(key)` 가 추가되어 Detail 의 "처리 완료로 표시" 가 DB flip 성공 뒤 tray 원본까지 제거. 이전 Known gap "Detail 전이의 tray cancel 부재" 해소. live listener 부재 시 best-effort no-op 은 신규 Known gap 으로 기록. 관련 plan: `docs/plans/2026-04-20-silent-archive-drift-fix.md` Task 3, PR #126.
