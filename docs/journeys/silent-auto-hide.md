...
5. 루트 요약 갱신: count 가 직전 값과 다르면 `SilentHiddenSummaryNotifier.post(count)` 호출 (동일 값 연속시 생략). PROCESSED 전이로 ARCHIVED count 가 줄어도 여기서 갱신된다. `smartnoti_silent_summary` 채널(IMPORTANCE_MIN) 로 다음 속성의 알림 게시:
   ...
   - **Visual identity** (PR #521, #510): smallIcon = `R.drawable.ic_replacement_silent` (volume_off 글리프, action 식별), largeIcon = **omitted** when summary aggregates multiple source packages (mixed-source guard) — root summary spans the whole archived cohort across apps so no single app icon represents it faithfully.
...
7. 그룹 요약 posting: planner 가 돌려준 `summaryPosts` / `childPosts` 에 대해 `SilentHiddenSummaryNotifier.postGroupSummary(key, count, preview, rootDeepLink)` + `postGroupChild(notificationId, entity, key)` 를 main dispatcher 에서 실행. 모두 `smartnoti_silent_group` 채널(IMPORTANCE_MIN), `setGroup(groupTagFor(key))` + summary 는 `setGroupSummary(true)`. 그룹 summary 의 title 은 `"{sender 또는 package} · 조용히 {N}건"`, InboxStyle 로 최근 preview 최대 3줄. Summary `contentIntent` 는 `Routes.Hidden.create(sender=...)` 또는 `packageName=...` 딥링크. **Visual identity** (PR #521, #510): 두 빌더 모두 smallIcon = `R.drawable.ic_replacement_silent` (action 식별). **Group summary** largeIcon: `SilentGroupKey.App(pkg)` 그룹은 `AppIconResolver.resolve(pkg)` 로 source 앱 launcher 아이콘 (resolver 가 null 을 반환하면 omit). `SilentGroupKey.Sender(...)` 그룹은 single-package children 일 때만 그 packageName 으로 large 를 채우고, 여러 packageName 이 섞여 있으면 omit. **Group child** largeIcon: 각 child 의 자기 packageName 으로 항상 resolve 시도. **루트 요약과 동일한 auto-dismiss timeout 적용** — group summary, group child 의 builder 모두 `ReplacementNotificationTimeoutPolicy` 결과를 동일 minutes 값으로 받으므로 timeout 으로 한꺼번에 사라진다. 직후 새 SILENT 가 들어와 N≥2 가 다시 충족되면 planner 가 다시 게시.
...
- `notification/SilentHiddenSummaryNotifier` — 루트 요약 (`smartnoti_silent_summary`) + 그룹 summary/child (`smartnoti_silent_group`) 채널 · 빌더 · PendingIntent · `groupTagFor` / `groupSummaryNotificationIdFor` / `groupChildNotificationId` · 세 builder 모두 `ReplacementNotificationTimeoutPolicy` 결과로 `setTimeoutAfter` 적용. **#510 (PR #521)**: 세 builder 모두 smallIcon = `ReplacementActionIcon.SILENT.drawableRes` 사용 + `appIconResolver: AppIconResolver` 주입을 통해 single-source 케이스에서 largeIcon = source app launcher icon 으로 채움 (mixed-source / unresolvable 은 omit).
- `notification/AppIconResolver` (PR #521) — PackageManager → Drawable → Bitmap conversion + `LruCache<String, Bitmap>(64)` 메모이즈 + install/upgrade/remove 인밸리데이션. Returns null when the source app exposes no displayable launcher icon — notifier omits `setLargeIcon` rather than masking with SmartNoti brand. Plan: `docs/plans/2026-04-27-fix-issue-510-replacement-icon-source-action-overlay.md`.
- `notification/ReplacementActionIcon` (PR #521) — compile-time enum: SILENT → `R.drawable.ic_replacement_silent`. Eliminates magic-int sprawl across notifier builders. Same plan as `AppIconResolver`.
...
- `data/local/TrayOrphanCleanupRunner` (#524) — 사용자 trigger 단일 pass 로 `smartnoti_silent_group_app:<pkg>` 기반 source orphan 일괄 cancel; PERSISTENT_PROTECTED skip; idempotent. Settings → "트레이 정리" 카드의 backend.
- `data/local/ActiveTrayInspector` (+ `ListenerActiveTrayInspector`, `ActiveTrayEntry`) (#524) — read-only port, 현재 트레이 snapshot 을 (key, packageName, groupKey, flags) projection 으로 노출 (`ActiveSourceNotificationInspector` 와 다른 사용 — runner 가 활성 snapshot 을 직접 순회해 source packageName set 을 group key 에서 유도해야 함)
- `ui/screens/settings/SettingsTrayCleanupSection` + `SettingsTrayCleanupSummaryBuilder` (#524) — Settings 카드 UI + preview 한 줄 빌더 (Android-free)
- `data/settings/SettingsSuppressionRepository.Keys.TRAY_CLEANUP_SKIP_CONFIRM_V1` (#524) — confirm-dialog "다시 묻지 않기" persisted preference (default false)
...
- **트레이 정리 (#524) ADB 검증 보류 (R3CY2058DLJ 미연결)** — Settings → "트레이 정리" 카드 코드 경로는 unit test 로 GREEN (`TrayOrphanCleanupRunnerTest`, `SettingsTrayCleanupSummaryBuilderTest`) 이지만 100+ source orphan 이 실제로 누적된 reference device (R3CY2058DLJ) 가 본 PR 시점에 연결돼 있지 않아 plan §Task 5 의 before/after `dumpsys` 비교가 미실행. PR body 에 recipe 가 그대로 보존되어 다음 sweep 에서 재실행 가능. 연결 plan: `docs/plans/2026-04-28-fix-issue-524-tray-orphan-cleanup-button.md` (Task 5 deferred row).
...
- 2026-04-27: **Issue #510 closure — replacement notifications carry source-app large icon + action-specific small icon glyph**. `SilentHiddenSummaryNotifier` 의 root summary / group summary / group child 세 builder 모두 smallIcon = `R.drawable.ic_replacement_silent` (volume_off 글리프, action 식별) 로 통일되고, `AppIconResolver` 가 single-source 케이스에서 source 앱 launcher 아이콘을 largeIcon 으로 채운다 (mixed-source root summary / multi-package Sender 그룹 / unresolvable 은 omit). Tasks 1-4 shipped via PR #521 (commit `cfe60e5`); Tasks 5-7 (ADB e2e + journey bumps + plan flip status:shipped) shipped via this PR. Live ADB verification on R3CY2058DLJ (debug APK rebuilt 2026-04-27 22:30 UTC): `dumpsys notification --noredact` confirms (a) every `smartnoti_silent_*` channel notification carries `icon=Icon(typ=RESOURCE pkg=com.smartnoti.app id=0x7f070099)` (= `ic_replacement_silent` per `aapt dump --values resources`), (b) Gmail-grouped child rows (`groupKey=smartnoti_silent_group_app:com.google.android.gm`) carry `android.largeIcon=Icon (Icon(typ=BITMAP size=101x101))` resolved through `AppIconResolver`, (c) mixed-source root summary id=23057 (`smartnoti_silent_summary`) carries `android.largeIcon=null` confirming the omission contract. Pre-fix, all SmartNoti silent notifications shared a single static `android.R.drawable.ic_menu_view` smallIcon and never set largeIcon — the tray collapsed multiple sources into one ambiguous SmartNoti brand row; now each row visually identifies both its source app (large) and the SmartNoti action that produced it (small) at a glance. Plan: `docs/plans/2026-04-27-fix-issue-510-replacement-icon-source-action-overlay.md` flipped to `status:shipped`. `last-verified` 2026-04-27 unchanged (same-day verification).
- 2026-04-28: **Issue #524 — Settings → "트레이 정리" 카드 (pre-#511 source orphan 일괄 cancel)**. `TrayOrphanCleanupRunner` (data.local) + `ActiveTrayInspector` 포트 + `SettingsTrayCleanupSection` Composable 추가. 사용자가 Settings 진입 시 카드 본문에 "원본 알림 N건 정리 가능 (앱 …)" 라이브 미리보기, "모두 정리하기" 버튼 → confirm 다이얼로그 (`tray_cleanup_skip_confirm_v1` boolean, default false; "다시 묻지 않기" 체크박스 only writer) → IO 코루틴으로 `runner.cleanup()` 가 활성 `StatusBarNotification` 단일 pass 로 (a) `smartnoti_silent_group_app:<pkg>` group key 에서 source packageName set 추출, (b) 그 set 에 속한 비-SmartNoti entry 만 후보로, (c) `FLAG_FOREGROUND_SERVICE | FLAG_NO_CLEAR | FLAG_ONGOING_EVENT` 교차 entry 는 PERSISTENT_PROTECTED 로 skip 후 logcat warn 한 줄, 나머지는 `SourceCancellationGateway.cancel(key)` 위임. 결과는 `(cancelledCount, skippedProtectedCount, notBound)` 토스트 + LaunchedEffect 가 미리보기 재호출해 "0건" 으로 즉시 갱신. `MigrateOrphanedSourceCancellationRunner` (#511) 가 못 잡은 cohort — `replacementNotificationIssued = 0` 으로 저장된 row 들 — 를 사용자 의지로 정리하는 보완 경로. 자동 sweep 아님 (사용자 trigger 만; trust 형성). DataStore 마이그레이션 / 새 채널 / 새 cancel API 없음. **Code pointers added**: `data/local/TrayOrphanCleanupRunner`, `data/local/ActiveTrayInspector` (+ `ListenerActiveTrayInspector`), `data/local/ActiveTrayEntry`, `ui/screens/settings/SettingsTrayCleanupSection`, `ui/screens/settings/SettingsTrayCleanupSummaryBuilder`, `data/settings/SettingsSuppressionRepository.Keys.TRAY_CLEANUP_SKIP_CONFIRM_V1`. **Known gap added**: "트레이 정리 ADB 검증 보류 (R3CY2058DLJ 미연결)" — unit test GREEN 이지만 100+ orphan reference device 미연결 상태에서 plan §Task 5 의 before/after `dumpsys` 비교는 다음 sweep 으로 연기. 관련 plan: `docs/plans/2026-04-28-fix-issue-524-tray-orphan-cleanup-button.md` (Tasks 1-3 shipped via PR #530, Tasks 4 + 6-7 via this PR; Task 5 deferred).
