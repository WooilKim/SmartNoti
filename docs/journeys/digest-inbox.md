...
- (resolved 2026-04-27, plan `docs/plans/2026-04-27-digest-empty-state-suppress-opt-in-cta.md`) Digest 알림이 0건이면 empty-state 만 보여주는데, suppress opt-in 유도 같은 교육용 문구 없음.
...
- 2026-04-27: Plan `2026-04-27-inbox-sort-by-priority-or-app` ship — `Routes.Digest` deep-link 진입 경로 (`DigestScreen` standalone) 도 `SmartNotiSettings.inboxSortMode` 를 collect 해서 `InboxSortPlanner.sortGroups(...)` 적용. Inbox 통합 탭에서 사용자가 선택한 정렬 모드가 settings 에 영속되므로 deep-link 로 다시 진입할 때도 동일 정렬을 본다. `Routes.Digest` composable 자체의 진입 경로 / 그룹 contract 는 무변경 — 정렬 helper 만 추가. 활성 contract 는 [inbox-unified](inbox-unified.md).
- 2026-04-27: Plan `2026-04-27-digest-empty-state-suppress-opt-in-cta` ship — `DigestScreen` 의 `groups.isEmpty()` 분기 (legacy `Routes.Digest` deep-link 진입 경로 포함) 의 empty-state subtitle 카피를 suppress opt-in 의미를 담는 문구 (`숨길 앱으로 지정한 앱의 반복 알림을 여기에 모아드려요`) 로 교체 + `EmptyState` 의 `action` 슬롯에 `FilledTonalButton(label="숨길 앱 선택하기", onClick=onOpenSuppressedAppsSettings)` 주입. `DigestScreen` 시그니처에 required `onOpenSuppressedAppsSettings: () -> Unit` 추가 — 두 진입 경로가 동일 lambda 로 `Routes.Settings` 에 launchSingleTop 으로 navigate. 카피/라벨 단일화는 `DigestEmptyStateAction` 상수 object + `DigestEmptyStateContractTest` 가 contract 고정. Normal state (groups.isNotEmpty()) 는 무변경. 활성 contract 는 [inbox-unified](inbox-unified.md).
- 2026-04-27: Rotation sweep (deprecated journey, oldest by `last-verified`) post-PR #491 (PROMO_QUIETING action SILENT→DIGEST + M1 v9→v10 migration). emulator-5554, APK `lastUpdateTime=2026-04-27 15:32:46` (post-#491 merge 15:31:43Z; #492 ContentSignatureNormalizer + #485 DiagnosticLogger both default-OFF so no behavior change for this journey). Recipe: 4× `cmd notification post -S bigtext -t Coupang Deal{i}` (모두 `pkg=com.android.shell`; PRIORITY 사용자 룰 때문에 PRIORITY 로 분류되어 DIGEST 그룹 카운트는 4 그대로) → BottomNav `정리함` (407,2274) → inbox-unified host 의 Digest 서브탭 default. **Observable steps 1-5 + Exit state 모두 일치**: `DigestScreen` 렌더 (Embedded mode), `observeDigestGroupsFiltered` collect, `toDigestGroups` packageName grouping (`Shell · 3건` + `SmartNoti Test Notifier · 1건`), `DigestGroupCard` (appName + `3건` count badge + `Shell 관련 알림 3건` summary + `최근 묶음 미리보기` + 3× NotificationCard preview), preview row tap (540, 1330) → `알림 상세` + `왜 이렇게 처리됐나요?` + `전달 모드 · Digest 묶음 전달` (Routes.Detail.create deep-link). **Bulk actions 무회귀**: 스크롤 후 expanded 영역 하단에 `모두 중요로 변경` + `모두 지우기` 두 OutlinedButton 행 노출 (2026-04-26 ship 그대로). **Sort dropdown 무회귀**: 헤더 우측 `정렬: 최신순` chevron tap → 메뉴 3옵션 (`최신순` / `중요도순` / `앱별 묶기`). **PR #491 PROMO 회귀 없음**: DB 의 PROMO-tagged row (`광고|30%`, `오늘만 특가 안내` 등) 가 `status=DIGEST` 로 정확히 저장되어 Shell + SmartNoti Test Notifier DIGEST 그룹에 정상 흡수, 카드 안에 `Digest` chip + `프로모션 알림` reason chip 표시. `Routes.Digest` standalone composable contract (DigestScreenMode.Standalone default) 무손상. DRIFT 없음. `last-verified` 2026-04-26 → 2026-04-27.
