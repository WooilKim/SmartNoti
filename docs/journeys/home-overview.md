...
- (resolved 2026-04-27) "code health: HomeScreen.kt 877 lines" — split into per-section sub-files via plan `docs/plans/2026-04-27-refactor-home-screen-split.md` (see Change log).
...
- 2026-04-27: Post-#425 (digest empty-state CTA) / #430 (uncategorized-prompt APP-Rule) / #433 (ignored-archive bulk) re-verification (emulator-5554, fresh APK `lastUpdateTime=2026-04-27 11:56:47`). 3 test notifications posted via shell, Home 탭 dump — Step 3 ScreenHeader OK, Step 4 StatPill "오늘 알림 45개 중 중요한 19개를 먼저 전달했어요" + `즉시 19 / Digest 4 / 조용히 22` live, Step 5 PassthroughReviewCard "검토 대기 19" + "검토하기" mount, Step 6 connected InlineRow "연결됨" + "실제 알림이 연결되어 있어요", Step 8 InsightCard "Shell 알림 15개..." + 3 reason chips (조용히 처리 8 / 사용자 규칙 1 / 프로모션 알림 1), Step 9 TimelineCard "최근 흐름" + 3h/24h 토글 + "최근 3시간 기준 15개" + 흐름/피크 segments, Step 10 최근 알림 5건 + `HomeRecentMoreRow` "전체 45건 보기" footer 정확 렌더 (DEFAULT_CAPACITY=5 cap 동작), Step 11 BottomNav 4-tab. Step 2 (uncategorized prompt) 비표시 = 24h snooze 잔존 (intended), Step 7 (QuickStartAppliedCard) 비표시 = ack/TTL consumed (intended). 최근 ship 3건은 Home rendering 에 직접 영향 없음 (Inbox empty-state / detector test wiring / archive bulk-action 영역). DRIFT 없음. `last-verified: 2026-04-26 → 2026-04-27`.
- 2026-04-27: Refactor — `HomeScreen.kt` (877 lines) split into 4 sub-files in the same package: `HomeInsightSection.kt` (InsightCard / ReasonBreakdownChart / InsightLinkText), `HomeTimelineSection.kt` (TimelineCard / TimelineBarChart / TimelineBar / TimelineRangeChip / SmartTimelineCardContainer / StatPill), `HomeNotificationAccessSection.kt` (HomeNotificationAccessInlineRow / HomeNotificationAccessCard), `HomeRecentMoreRow.kt`. Root file 877 → 401 lines. Composable visibility flipped `private` → `internal`; no behavior change — characterization tests pinned by PR #461 stay GREEN, full unit suite green. Plan `docs/plans/2026-04-27-refactor-home-screen-split.md` (status: shipped). `last-verified` not bumped — refactor only, no observable behavior change to re-verify.
