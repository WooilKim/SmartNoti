...
- 2026-04-27 (journey-tester re-attempt, post #430 wiring): UTC 05:54Z 시점 재시도 — caller note 와 달리 snooze epoch 가 동일 (1777274058159 = 07:14:18Z) 이라 여전히 ~1h20m 잔존. `am start … MainActivity` + `uiautomator dump` 로 Home 화면에서 `분류 제안 / 새 앱 / 분류 만들기 / 나중에` 문자열 모두 부재 확인 (snooze 로 인한 의도된 None). DataStore cross-check: `smartnoti_rules.preferences_pb` 에 `APP%3Acom.android.shell` rule 존재 + `cat-onboarding-important_priority` 의 인코딩된 `ruleIds` 에 `APP%253Acom.android.shell` 포함 — 즉 `com.android.shell` 은 신규 #430 detector path 에서 APP-Rule 경유로 coverage 합집합에 들어간 상태 (Category.appPackageName 은 비어 있음). `:app:testDebugUnitTest --tests UncategorizedAppsDetectorTest --rerun-tasks` 12/12 GREEN 재확인. 시각적 "APP-Rule 만으로 분류된 앱이 카드에서 빠진다" live 검증은 snooze expire (07:14Z 이후) 또는 destructive 우회 필요 — `last-verified` 미bump (2026-04-26 유지).
- 2026-04-27 (journey-tester full sweep, post-snooze-expiry): UTC 08:08Z (snooze 07:14Z 만료 ~54분 경과) 에 `am force-stop` + 재기동 후 `uiautomator dump` → 카드 4요소 모두 렌더 PASS: ContextBadge "분류 제안", title "새 앱 3개가 알림을 보내고 있어요", body "SmartNotiDebugTester, SmartNoti Test Notifier, Coupang 앱을 분류하시겠어요?", "분류 만들기" + "나중에" 버튼. **#430 APP-Rule coverage 시각 검증**: body sample 3개에 `com.android.shell` 부재 — DataStore 의 `cat-onboarding-important_priority` Category 가 `APP:com.android.shell` Rule 을 ruleIds 로 참조한 상태가 detector 합집합에 정확히 반영됨 (이전 sweep 들이 미검증으로 남겼던 path 가 live 로 PASS). "나중에" 탭 → 카드 즉시 unmount + DataStore `uncategorized_prompt_snooze_until_millis` 가 1777274058159 → 1777363800866 (2026-04-28T08:10:00Z, +24h) 로 persist 확인. `last-verified` 2026-04-26 → 2026-04-27 bump.
